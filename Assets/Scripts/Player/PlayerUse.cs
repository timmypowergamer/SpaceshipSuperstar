...
public class PlayerUse : MonoBehaviour {
	...
	public PlayerPawn Player
	{
		get
		{
			if (m_player == null)
			{
				m_player = GetComponent<PlayerPawn>();
			}
			return m_player;
		}
	}
	...
	void Update () {
		if (Input.GetButtonDown("Use"))
		{
			if (Player.GrabbedObject == null)
			{
				...
				if (Physics.Raycast(Player.Cam.transform.position, Player.Cam.transform.forward, out hit, 5f))
				{
					if (hit.transform.GetComponent<GrabableObject>() != null)
					{
